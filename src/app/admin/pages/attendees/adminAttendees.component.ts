...
@Component({
  selector: 'app-adminAttendees',
  templateUrl: './adminAttendees.component.html',
  styleUrls: ['./adminAttendees.component.scss'],
  animations: [
    trigger('slideInOut', [
      state('in', style({
        overflow: 'hidden',
  ...
  deleteUser(id, name) {
    try {
      ...
      this.confirmDialogSrv.confirmThis("確認要刪除此成員", () => {
        this.attendeeSrv.deleteUser(id, this.currentUser.id).subscribe(res => {
